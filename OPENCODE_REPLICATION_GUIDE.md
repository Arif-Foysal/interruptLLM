Here is the updated guide, fully modified to target **OpenCode** instead of Claude Code. All references to Claude, its context window, and `CLAUDE.md` have been adapted to OpenCode and `OPENCODE.md`.

--- START OF MODIFIED FILE ---

Pipeline Replication Guide — Read This and Build It
Instructions to OpenCode: This file is a self-contained spec for an OpenCode + Kaggle ML-Ops
pipeline. When a human drops this file into a fresh repository and asks you to “set up the pipeline,” your job is to:
...
4. Hand off — from then on, your operating instructions live in the `OPENCODE.md` you just created.

Do not start writing experiment code. Your job here is the harness, not the science. The human (or a follow-up
session) will fill in `src/` and `templates/`.
...
| 5 | One-paragraph project description | “Multi-lead ECG analysis with sheaf theoretic persistent homology...” Goes into `OPENCODE.md`/`AGENTS.md` so future sessions have context |
...
└──────┬───────┘
       │
┌──────▼───────┐
│   OpenCode   │ edits notebooks · interprets · iterates
└──┬───────┬───┘
   push│   │fetch
┌──▼───────▼───┐
...
*   `src/` is uploaded as a single Kaggle dataset, not vendored. Notebooks import `<core>` after locating the mounted dataset path. One source of truth; one `upload-src` to update everywhere.
*   `templates/` is checked-in and pristine; `notebooks/` is mutable and gitignored. Templates are the “blueprint”; notebooks are the working copy you edit. This keeps diffs tractable.
*   Phase dependencies live in `config.yaml`, not in `pipeline.py`. Each notebook lists which datasets it needs (raw + intermediate-results-from-prior-phases). The CLI reads this list when building `kernel-metadata.json`.
*   Three result conventions: `[RESULT] key = value` lines in stdout (greppable), `<phase>_results.json` saved to `kaggle/working/`, and PNG plots. That’s the contract that lets the agent judge “did this work.”
...
#### 3.8 OPENCODE.md
Operating instructions for every future OpenCode session in this repo.
```markdown
...
python pipeline.py push notebooks/.py # token loads from .env
...
If you want a human-facing quickstart separate from `OPENCODE.md`, create one summarizing §2 + §4. Skip if the human says they don’t need it.
...
| `fetch` succeeds but no `[RESULT]` lines printed | Notebook didn't call `core.format_result(...)` | Make sure every metric goes through the wrapper |
...
>
> `OPENCODE.md` has the operating instructions for future sessions. Just point OpenCode at it.
...
The whole point of the harness is that an LLM (OpenCode) drives it. Every byte the harness emits ends up in OpenCode's context window — and prompt-caching only helps the static part of context. Treat agent-facing output as a budget.
...
- `pipeline.py tail <kernel> [N]` — last N lines of the kernel log (default 40). Never cat a Kaggle log into OpenCode's context; logs are JSON-Lines and routinely exceed 1 MB.
- `_MAX_RESULT_LINES = 200` cap in `_parse_file` — bounds how many `[RESULT]` lines any single command surfaces.

Rules the agent (you, OpenCode) must follow:
...
- Keep `OPENCODE.md`, `src/`, `templates/` stable across iterations. They form the cacheable prefix of every session in this repo. Edit them only when the change is durable; don’t rewrite them per experiment.
